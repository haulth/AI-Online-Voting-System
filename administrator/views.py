...
def import_csv(request):
    ...
    return JsonResponse(
        {"success": False, "errors": "Invalid request method or missing file"}
    )
...
def import_csv_data(csv_file):
    ...
    for row in data:
        if len(row) != 6:  # Có 6 cột trong tệp CSV
            raise ValueError("Số cột trong file CSV không đúng")

        ho, ten, email, password, sdt, anh_url_or_path = row
...
>>>>>>> 45b7fa4cca34c31607dfb7b1421630e6892a42a4
            ...
            # Kiểm tra nếu là URL hoặc đường dẫn cục bộ
            if anh_url_or_path.startswith("http://") or anh_url_or_path.startswith("https://"):
                # Tải ảnh từ URL
                response = requests.get(anh_url_or_path)
                if response.status_code == 200:
                    with open(image_path, "wb") as f:
                        f.write(response.content)
                else:
                    raise ValueError(f"Không thể tải ảnh từ URL: {anh_url_or_path}")
            else:
                # Sao chép ảnh từ đường dẫn cục bộ
                if os.path.isfile(anh_url_or_path):
                    with open(anh_url_or_path, "rb") as src_file:
                        with open(image_path, "wb") as dst_file:
                            dst_file.write(src_file.read())
                else:
                    raise ValueError(f"Đường dẫn ảnh cục bộ không hợp lệ: {anh_url_or_path}")

            # Đăng ký tài khoản với thông tin và ảnh đã lưu
            response = account_register_auto(email, password, ho, ten, sdt, image_path)
...
def account_register_auto(email, password, first_name, last_name, phone_number, image_path):
    if not CustomUser.objects.filter(email=email).exists():
        ...
        voterForm = VoterForm(voter_data)
...
>>>>>>> 45b7fa4cca34c31607dfb7b1421630e6892a42a4
            ...
            with open(image_path, "rb") as img_file:
                with open(image_full_path, "wb") as dest_file:
            ...
            errors = {**userForm.errors, **voterForm.errors}
            return JsonResponse({"success": False, "errors": errors})
    else:
        return JsonResponse({"success": False, "errors": {"email": "Người dùng đã tồn tại"}})
...
def account_register(request):
    ...
    context = {"form1": userForm, "form2": voterForm, "page_title": "Đăng Ký Tài Khoản"}
...
def ad_train(request):
    context = {"page_title": "Huấn Luyện Mô Hình Máy Học"}
    return render(request, "admin/ad_train.html", context)


def interface(request):
    context = {"page_title": "Điểm Danh"}
...
def timetrain(request):
    ...
    train_datetimes = [datetime.strip() + "<br>" for datetime in train_datetimes]
...
def save_vote_time(request):
    if request.method == "POST":
        ...
        if not request.POST.get("vote_time_start") or not request.POST.get(
            "vote_time_end"
        ):
        ...
        if vote_datetime_end < vote_datetime_start:
            messages.error(
                request,
                "Thời gian kết thúc bầu cử phải lớn hơn thời gian bắt đầu bầu cử.",
            )
            return redirect(reverse("viewPositions"))
            ...
            if len(candidate_data) < 1:
                ...
                if position.max_vote > 1:
                    ...
                    if winner["votes"] == 0:
                        ...
                        if count > 1:
                            winner = (
                                f"Có {count} đại biểu với {winner['votes']} phiếu bầu"
                            )
                        else:
...
def infoVoter(request):
    try:
        ...
        # Nếu thời gian bình chọn đã bắt đầu
            ...
            time_left_str = (
                f"{int(time_left.days * 24 + hours)}:{minutes:02d}:{seconds:02d}"
            )
...
def QRVoter(request):
    ...
    try:
        ...
        image_path = os.path.join(
            "static", "images", "qrcode", str(admin.get_id()) + ".png"
        )
...
def deleteVoter(request):
    ...
    try:
        ...
        try:
            image_path = os.path.join(
                "static", "image", "qrcode", str(admin.get_id()) + ".png"
            )
            full_path = os.path.abspath(image_path)
            os.remove(full_path)
        ...
        for root, dirs, files in os.walk(folder_path, topdown=False):
            for name in dirs:
                ...
                try:
                    ...
                    pass
        try:
            folder_path = os.path.join("static", "data_process", "process")
            # Lặp qua tất cả các thư mục và tệp tin trong folder_path
            for root, dirs, files in os.walk(folder_path, topdown=False):
                for name in dirs:
                    # Xóa thư mục
            ...
            pass
        try:
            folder_path = os.path.join("static", "data_process", "raw")
            # Lặp qua tất cả các thư mục và tệp tin trong folder_path
            for root, dirs, files in os.walk(folder_path, topdown=False):
                for name in dirs:
                    # Xóa thư mục
...
def viewPositions(request):
    ...
    context = {"positions": positions, "form1": form, "page_title": "Loại bình chọn"}
    if request.method == "POST":
        if form.is_valid():
...
def viewCandidates(request):
    ...
    context = {"candidates": candidates, "form1": form, "page_title": "Đại biểu"}
...
def ballot_position(request):
    context = {"page_title": "Vị trí lá phiếu"}
...
def ballot_title(request):
    ...
    try:
        ...
        file.write(title)
        file.close()
        messages.success(request, "Tiêu đề bầu cử đã được thay đổi thành " + str(title))
        return redirect(url)
    except Exception as e:
...
def viewVotes(request):
    ...
    context = {"votes": votes, "page_title": "Phiếu bầu"}
